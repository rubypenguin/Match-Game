...
public class LanguagesManager {
	...
	private LanguagesManager() {
		...
		HashMap<String, String> systemLanguage = _strings.get(java.util.Locale
				.getDefault().toString());
	...
	private void loadLanguagesFile() {
		try {
			...
			Array<Element> languages = root
					.getChildrenByNameRecursively("language");

			for (int i = 0; i < languages.size; ++i) {
				// Get language name and whether if it is the default one or not
				...
				Array<Element> strings = languageElement
			...
			System.out
					.println("Error loading languages file " + LANGUAGES_FILE);
		}
	}
}
